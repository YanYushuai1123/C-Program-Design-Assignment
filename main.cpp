...
string arbitraryprecision(string, int, int, int);
...
string getresult(string a) {
    ...
    /* while (true) {
          ...
          if (sub == string::npos)
              break;
          string su1 = a.substr(sub - 1, 1);
          string su2 = a.substr(sub + 1, 1);
          int sub1 = atoi(su1.c_str()), sub2 = atoi(su2.c_str());
          if(sub1<sub2)
          {
              string s = convertToString(abs(sub1 - sub2));
          ...
          string s = convertToString(sub1 - sub2);
          a = replace(a, s, sub);
